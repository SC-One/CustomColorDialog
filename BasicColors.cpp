...

{
  ...
  for (int i = 0; i < _colorsCount; ++i) {
    _gridLayout.addWidget(_basicColors[i].get(), i % row, i % col);
  }
}

void BasicColors::initBasicColors() {
  for (int i = 0; i < _colorsCount; ++i) {
    std::unique_ptr<ClickableLabel> tempLabel(new ClickableLabel(this));
    connect(tempLabel.get(), &ClickableLabel::clicked, this,
            &BasicColors::setCurrentColor);
    QColor color(RAND_COLOR(0, 256), RAND_COLOR(0, 256), RAND_COLOR(0, 256));
    tempLabel->setColor(color);
    _basicColors.push_back(std::move(tempLabel));
  }
}
...
BasicColors::TwoDivisor BasicColors::biggestDivisors() const {
  ...
  int min =
      DIFF_DIVISORS(listOfDivisors[0].divisor1, listOfDivisors[0].divisor2);
  size_t indexMin = 0;
  for (size_t i = 0; i < listOfDivisors.size(); ++i) {
    auto tempDiff =
        DIFF_DIVISORS(listOfDivisors[i].divisor1, listOfDivisors[i].divisor2);
    if (tempDiff < min) {
      min = tempDiff;
      indexMin = i;
    }
...
QColor BasicColors::currentColor() const { return _currentColor; }
...
const QString BasicColors::title() const { return _title.text(); }

void BasicColors::setTitle(const QString& title) { _title.setText(title); }
